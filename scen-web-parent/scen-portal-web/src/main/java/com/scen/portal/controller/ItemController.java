...
public class ItemController {
    ...
    public String showItem(@PathVariable Long itemId, Model model) {
        ItemInfo item = null;
        String itemBaseInfo = itemCacheService.getItemBaseInfo(itemId);
        if (StringUtils.isBlank(itemBaseInfo)) {
            Item poItem = itemService.getItemById(itemId);
            itemBaseInfo = JsonUtils.objectToJson(poItem);
            itemCacheService.setItemBaseInfo(poItem);
        }
        item = JsonUtils.jsonToPojo(itemBaseInfo, ItemInfo.class);
